...
export const resizeLocal = async (bucket, fromFileFullPath, toFileFullPath, size, contentType) => {
...
export const blendLocal = async (mapFilePath, photoPath, iconPath,size, toFileFullPath, contentType) => {
  //const tmpResizeFile = path.join(os.tmpdir(), UUID());
  try {
    // resize
...
export const resizedImage = async (object, toFileFullPath, size) => {
  ...
  const ret = await resizeLocal(bucketObj, fromTempFilePath, toFileFullPath, size, object.contentType);
