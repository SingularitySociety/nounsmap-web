...
export const posted = async (
  ...
  const fromObj = {
    ...
    name: "images/photos/" + photoId + "/original.jpg",
  };
  const toObj = {
    bucket: firebaseConfig.storageBucket,
    name: "images/photos/" + photoId + "/ogp/600.jpg",
    context: "image/jpeg",
  };
  try {
...
export const debugPhotoSync = async (
  db,
  data: any,
  context: functions.https.CallableContext | Context
) => {
  ...
  const url =
    process.env.FUNCTIONS_EMULATOR && process.env.FIRESTORE_EMULATOR_HOST
      ? `http://localhost:9199/v0/b/${bucketObj.name}/o/${encodeURIComponent(
          file.name
